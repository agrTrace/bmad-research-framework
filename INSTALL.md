...
> ℹ️ Electron 的安装脚本需要联网下载平台二进制。仓库在 `desktop/electron/.npmrc` 中预置了 `npmmirror.com` 镜像地址，用于缓解国内或受限网络环境下的 `ECONNRESET`、下载超时等问题。如仍报错，可临时设置 `ELECTRON_MIRROR=<mirror>` 或清理 `node_modules` 后重试。
